...
const nameInventor = inventors.map(
  (inventor) => inventor.first + " " + inventor.last
);
console.log(nameInventor);
// Array.prototype.sort()
// 3. Sort the inventors by birthdate, oldest to youngest
const ordered = inventors.map((inventor) => inventor.year).sort();
console.log(ordered);
...
const peopleOrdered = people.sort(function (lastOne, nextOne) {});
...
const reduceData = data.reduce(function (obj, item) {
  if (!obj[item]) {
    obj[item] = 1;
  }
  obj[item] = obj[item] + 1;
